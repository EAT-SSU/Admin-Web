...
function Meal({ date, timePart }) {
    ...
    const fetchMealsData = async () => {
        try {
            setIsLoading(true);
            const response =
                await sendAuthorizedHttpRequest('get', `/meals?date=${formatedDate}&timePart=${timePart.toUpperCase()}`);
        ...
        <div className='menuBoardsContainer'>
            {isLoading ? (
                ...
                menuBoardDatas.map((menuBoardData) => (
                    <Restaurant menuType={'meal'} isLoading={isLoading} boardData={menuBoardData} addMealAction={addMeal} deleteMealAction={deleteMeal} />
                ))
            )}
        </div>
    );
